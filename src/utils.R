...
select_prob <- function(p_value, strength){
...
simulate_ma_data <- function(required_trials, p_contr, odds_ratio, bias_type, bias_strength = NULL, tau = 0){
...
generate_ma <- function(job_id, scenario_id, bias_type, bias_percentage,
  ...
  if(heterogeneity > 0){
    ...
    required_trials <- obtain_true_ma_size(ma_size = ma_size,
                                           bias_type = bias_type,
                                           bias_percentage = bias_percentage)
    ma_data <- simulate_ma_data(p_contr = p_control,
                                 odds_ratio = odds_ratio,
                                 bias_type = bias_type,
                                 bias_strength = bias_strength,
                                 tau = tau)
